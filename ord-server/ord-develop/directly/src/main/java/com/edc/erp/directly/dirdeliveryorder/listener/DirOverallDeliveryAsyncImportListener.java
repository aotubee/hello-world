 ...
 */
@Slf4j
public class DirOverallDeliveryAsyncImportListener extends ImportListener<Map<Integer, String>> {
    ...
    public DirOverallDeliveryAsyncImportListener() {
...
//                    errorJoiner.add("第" + (column + NumberUtil.INTEGER_ONE) + "列配货数量不能为空");
                } else {
                    ...
                    if (NumberUtil.INTEGER_ZERO > deliveryQuantity || !cn.hutool.core.util.NumberUtil.isInteger(deliveryQuantityStr)) {
                        errorJoiner.add("第" + (column + NumberUtil.INTEGER_ONE) + "列配货数量必须为大于0的正整数");
                    }
                }
...
//                    errorJoiner.add("第" + (column + NumberUtil.INTEGER_ONE) + "列配货数量必须为正整数");
//                }
            }
        });
