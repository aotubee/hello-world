...
public class DssDirUpperLowerLimitServerImpl implements DssDirUpperLowerLimitServer {
        ...
        if (storeDssOrderGoodsMap.size() < EXECUTED_QUANTITY) {
            ...
            for (int i = 0; i < CORE_POOL_SIZE; i++) {
                ...
                threadPool.submit(new Runnable() {
                    @Override
                    public void run() {
                        log.info("当前线程待执处理的直营点三三智能跑货有{}条数据........................", finalNeedHandleStoreCodeMap.size());
...
//        StringJoiner storeCodesJoiner = new StringJoiner(SystemConstant.COMMA);
        storeDssOrderGoodsMap.entrySet().forEach(entry -> {
            ...
            entry.getValue().forEach(dssOrderInfoOut -> {
                ...
                goodsJoiner.add(dssOrderInfoOut.getSSpbh());
            });
            orderHandle.createUpAndDownOrder(entry.getKey(), SystemConstant.SYSTEM_USER, createOrderSkuInList, bizOrgCode, BusinessTypeColumnEnum.INTELLIGENT_UP_LOW_DOWN.getType());
        });
    }
