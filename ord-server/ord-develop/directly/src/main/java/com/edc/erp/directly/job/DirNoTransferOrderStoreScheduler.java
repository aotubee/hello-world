...
@Component
@Slf4j
public class DirNoTransferOrderStoreScheduler {
    ...
    @Autowired
    private DirOrderConfigHandle disOrderConfigHandle;
...
//    @Scheduled(cron = "0 0/10 * * * ?")
    public void dirSendMessageForNoTransferOrder() {
        ...
        log.info("开始执行查询合作经营未定中转单门店任务");
        if (taskLockUtils.lock(SystemConstant.DIRECTLY_NO_TRANSFER_ORDER_SWITCH, 60 * 3 * 1000)) {
            try {
                // 查询 APP用户
                List<String> storeCodeList = storeCenterService.findEnableAppStoreCodeListByPropertyAndOrg(StoreConstant.StoreProperty.DIRECTLY.getMytValue(), OrgCodeConvertEnum.XIAN_MYT.getBizOrgCode());
                if (CollectionUtils.isEmpty(storeCodeList)) {
                    log.info("合作经营未定中转单门店定时器未查到源门店");
                    return;
                }
                ...
                if (CollectionUtils.isNotEmpty(noTransferStoreCodeList)) {
                    ...
                    content = MessageFormat.format(NoticeNoTransferOrderStoreTemplateEnum.NO_TRANSFER_ORDER_ONE.getTemplateMessage(), "合作经营", noTransferStoreCodeList.size(), storeCodes);
                } else {
                    content = MessageFormat.format(NoticeNoTransferOrderStoreTemplateEnum.NO_TRANSFER_ORDER_TWO.getTemplateMessage(), "合作经营", "0");
                }
                    ...
                    if (StringUtils.isNotEmpty(noTransferOrderStoreUrl)) {
                        for (String url : noTransferOrderStoreUrl.split(",")) {
                            ...
                            if (null != response && response.isSuccess()) {
                                log.info("发送合作经营未下中转订单门店信息" + JSON.toJSONString(response));
                            }
                        }
                    }
                } catch (Exception e) {
                    log.error("发送合作经营未下中转订单门店信息异常", e);
                }
            } catch (Exception e) {
                log.error("执行查询合作经营未定中转单门店任务异常：", e);
            } finally {
                taskLockUtils.unlock(SystemConstant.DIRECTLY_NO_TRANSFER_ORDER_SWITCH);
            }
        } else {
            log.info("查询合作经营未定中转单门店定时器定时器锁还未释放：{}", SystemConstant.DIRECTLY_NO_TRANSFER_ORDER_SWITCH);
        }
        log.info("查询合作经营未定中转单门店任务执行结束");
    }
}
