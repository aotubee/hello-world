package com.edc.erp.directly.upperlowerlimit.server.impl;
...
import com.edc.erp.directly.upperlowerlimit.mapper.DirUpperLowerLimitMapper;
import com.edc.erp.directly.upperlowerlimit.model.out.InvStockStoreOut;
...
@Slf4j
@Service
public class DirUpperLowerLimitServerImpl implements DirUpperLowerLimitServer {

    @Autowired
    private DirUpperLowerLimitMapper upperLowerLimitMapper;
...
//                    }
                ...
                threadPool.submit(new Runnable() {
                    @Override
                    public void run() {
                        log.info("当前线程待执处理的直营跑货有{}条数据........................", finalNeedHandleStoreCodeList.size());
    ...
    private void executedJob(List<String> storeCodeList, String bizOrgCode) {
        ...
        for (String storeCode : storeCodeList) {
            try {
                ...
                String msg = MessageFormat.format(UpLowerLimitListWarningTypeEnum.UP_LOWER_LIMIT_NOT_RUNNING_CARGO.getErrorMessage(), StoreOrderWarningTypeEnum.DIRECTLY.getName(), storeCode, e.getMessage());
...
//                        msg, null, null, bizOrgCode);
        ...
        if (storeCodesJoiner.length() > 0) {
            String msg = MessageFormat.format(UpLowerLimitListWarningTypeEnum.UP_LOWER_LIMIT_NOT_RUNNING_CARGO.getErrorMessage(), StoreOrderWarningTypeEnum.DIRECTLY.getName(),
...
//                this.addErrorMessage(errorJoiner, replenishmentConfig.getSkuCode(), "不允许跑货");
                ...
                if (Objects.isNull(orderGoodsOut.getDistributionUnitPrice())) {
                    log.info("门店{},商品{}---{}", storeCode, replenishmentConfig.getSkuCode(), "配送价为空");
                    this.addErrorMessage(errorJoiner, replenishmentConfig.getSkuCode(), "配送价为空");
                    continue;
                }
            }
            // 查找订单类型配置
            ...
            DirOrderTypeConfig dirOrderTypeConfig = orderConfigHandle.getOneByCombinationTypeValueList(storeInfo.getErpStoreCode(), bizOrgCode, combinationTypeCodeList);
            ...
            if (Objects.isNull(storeDelivery)) {
                throw new BusinessException("门店" + storeInfo.getErpStoreCode() + "配送信息" + dirOrderTypeConfig.getOrderPeriod() + "为空");
            }
...
//            OrderCycleDeliveryOut orderCycleDeliveryOut = shoppingCartService.getStoreDeliveryType(storeLogisticsOut, dirOrderTypeConfig.getOrderPeriod());
            if (Objects.isNull(orderCycleDeliveryOut)) {
                throw new BusinessException("门店" + storeInfo.getErpStoreCode() + "要货周期为空");
            }
...
//                continue;
            ...
            List<InOneQtyVO> inOneQtyVOList = ordDirDeliveryService.findInDeliveryOrder(storeCode, beginTime, endTime);
            ...
            if (inventory.compareTo(replenishmentConfig.getInventoryLowerLimit()) < 1) {
                ...
                if (inventory.compareTo(BigDecimal.ZERO) == -1) {
...
//                    inTransitQuantity = goodsInTransitQuantityMap.get(replenishmentConfig.getSkuCode()).divide(BigDecimal.valueOf(orderGoodsOut.getDistributionSpecification().getQpc()), 0, RoundingMode.DOWN);
        ...
        if (storeOrderCartMap.size() < 1) {
            log.error("F-门店：{} 跑货异常: {}", storeCode, "无购物车商品数据");
            return false;
        }
        storeOrderCartMap.forEach((storeInfoId, orderCartOuts) -> {
            List<CreateOrderSkuIn> createOrderSkuInList = orderCartOuts.stream().map(orderCartOut -> {
                ...
                return createOrderSkuIn;
            }).collect(Collectors.toList());
            if (CollectionUtils.isEmpty(createOrderSkuInList)) {
                log.info("门店{}无跑货商品", storeCode);
                return;
            }
            orderHandle.createUpAndDownOrder(storeCode, SystemConstant.SYSTEM_USER, createOrderSkuInList, bizOrgCode, SourceTypeEnum.UPLOWDOWN.getKey());
            // 钉钉消息通知过滤的商品
            if (errorJoiner.length() > NumberUtil.INTEGER_ZERO) {
                String msg = MessageFormat.format(UpLowerLimitListWarningTypeEnum.UP_LOWER_LIMIT_FILTER_GOODS_NOTICE.getErrorMessage(),
                        StoreOrderWarningTypeEnum.DIRECTLY.getName(), storeCode, errorJoiner.toString());
                warningService.pushWarningMessage(WarningBusinessTypeEnum.UP_LOWER_LIMIT.getBusinessType(),
