package com.edc.erp.directly.returnorder.listener;
...
import com.edc.erp.directly.returnorder.enumeration.OrdReturnGoodsColumnEnum;
import com.edc.erp.directly.returnorder.model.excel.ImportOrdReturnGoodsVO;
import com.edc.erp.directly.returnorder.model.out.SaveReturnGoodsOut;
...
//            throw new BusinessException(OrdReturnGoodsColumnEnum.DISTRIBUTION_QUANTITY.getValue() + "不能为空;");
        ...
        if (Objects.isNull(goodsOut)) {
            errorJoiner.add(goodsCode + "商品不允许直营配货退货;");
//            throw new BusinessException(goodsCode + "商品不允许直营配货退货;");
        } else {
...
//                throw new BusinessException("商品代码已重复;");
            ...
            if (!distributionType.equals(goodsOut.getDistributionWay())) {
                errorJoiner.add(goodsCode + "商品配送方式与单头不一致;");
...
//            throw new BusinessException(goodsCode + "商品退货仓位与所选退货仓位不符;");
            ...
            if (Objects.isNull(warehousePrice)) {
                log.error("统配导入退货明细商品{}仓储库存价为空", goodsCode);
            ...
            BigDecimal sellTax = saveReturnGoodsOut.getSellTax() == null ? BigDecimal.ZERO : saveReturnGoodsOut.getSellTax().divide(new BigDecimal(SystemConstant.PERCENTAGE));
            //1+税率
            ...
            //退货税额
            saveReturnGoodsOut.setReturnTaxAmount(saveReturnGoodsOut.getAuditReturnAmount().subtract(saveReturnGoodsOut.getReturnExceptTaxAmount()));
            //仓储成本金额
