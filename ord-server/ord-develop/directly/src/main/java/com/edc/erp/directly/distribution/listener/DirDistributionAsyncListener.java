...
@Slf4j
@Data
public class DirDistributionAsyncListener extends ImportListener<ImportOrdOrderStoreGoodsVO> {
    ...
    public DirDistributionAsyncListener() {
    }
    ...
    public void invoke(ImportOrdOrderStoreGoodsVO data, AnalysisContext context) {
        ...
        if (errorJoiner.length() > 0) {
            ...
            OrdDirDistributionImportResultOut ordDirDistributionImportResultOut = importResultMap.get(data.getStoreCode());
    ...
    public Map<String, OrdDirDistributionImportResultOut> getImportResultMap() {
        return this.importResultMap;
    }

    @Override
    public void doAfterAllAnalysed(AnalysisContext context) {
        List<OrdDirDistributionImportGoodsOut> importList = Lists.newArrayList();
        importResultMap.values().forEach(ordDirDistributionImportResultOut -> {
            if(CollectionUtils.isEmpty(ordDirDistributionImportResultOut.getImportGoodsOutList())){
                return;
            }
            importList.addAll(ordDirDistributionImportResultOut.getImportGoodsOutList());
        });
        if (importList.size() >= BATCH_COUNT) {
            importResultMap.clear();
