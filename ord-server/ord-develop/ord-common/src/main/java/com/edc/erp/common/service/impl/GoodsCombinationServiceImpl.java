...
import com.edc.erp.common.mapper.GoodsCombinationMapper;
...
@Slf4j
public class GoodsCombinationServiceImpl extends BaseServiceImpl<GoodsCombination> implements GoodsCombinationService {
     
     private final GoodsCombinationMapper goodsCombinationMapper;
     ...
     public Integer getOrderTypeConfigIdByCombinationTypeValues(String storeCode, String bizOrgCode, List<String> combinationTypeValueList) {
          ...
          // 根据组织和订单类型id查找商品组合集合
          List<GoodsCombination> goodsCombinationList = goodsCombinationMapper.findByOrgCodeAndOrderTypeConfigIdList(bizOrgCode, orderTypeConfigIdList);
          if (CollectionUtils.isEmpty(goodsCombinationList)) {
               throw new BusinessException(storeCode + "未匹配到订单流商品组合配置");
          }
          Map<String, List<GoodsCombination>> combinationTypeCodeMap = goodsCombinationList.stream().collect(Collectors.groupingBy(GoodsCombination::getCombinationValue));
//          log.info("门店{}combinationType配置转MAP-----{}",storeCode, JSONObject.toJSONString(combinationTypeCodeMap));
          // 符合当前查询条件的商品组合集合
          List<GoodsCombination> eligibleGoodsCombinationList = Lists.newArrayList();
          ...
          Map<Integer, Long> countOrderTypeConfigIdMap = eligibleGoodsCombinationList.stream().collect(Collectors.groupingBy(GoodsCombination::getOrderTypeConfigId, Collectors.counting()));
