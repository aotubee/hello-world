package com.edc.erp.disordercart.controller;
...
@RequestMapping("/ord/ordDisOrderCart")
@Api(value = "ordDisOrderCart", tags = "配销购物车模块")
public class OrdDisOrderCartController {

    @Autowired
    private DisShoppingCartService disShoppingCartService;
    ...
    public Response<List<OrderCycleHeaderOut>> findMyOrderCartList() {
        ...
        List<OrderCycleHeaderOut> orderCycleHeaderOuts = disShoppingCartService.findMyOrderCartList(appUserOut);
    ...
    public Response batchDeleteOrderCart(@RequestBody List<DeleteOrderCartIn> deleteOrderCartInList) {
        String userName = UserUtil.getUserName();
        AppUserOut appUserOut = appUserService.getAppUserByUserCode(userName);
        if (Objects.isNull(appUserOut)) {
            return Response.error(SystemConstant.HTTP_UNAUTHORIZED, "不存在的用户");
        ...
        disShoppingCartService.deleteOrderCart(deleteOrderCartInList, appUserOut.getStoreCode(), bizOrgCode);
        return Response.success("删除成功");
    }

    @ApiOperation(value = "核算购物车金额", notes = "核算购物车金额", httpMethod = "POST")
    @PostMapping("/calculationOrderCartAmount")
    public Response<CalculationShoppingCartOut> calculationShoppingCartAmount(@RequestBody List<DisOperationShoppingCartIn> disOperationShoppingCartInList) {
        String userName = UserUtil.getUserName();
        AppUserOut appUserOut = appUserService.getAppUserByUserCode(userName);
        if (Objects.isNull(appUserOut)) {
            return Response.error(SystemConstant.HTTP_UNAUTHORIZED, "不存在的用户");
        }
        return Response.data(disShoppingCartService.calculationShoppingCartAmount(disOperationShoppingCartInList, appUserOut));
    }

    @ApiOperation(value = "更新购物车记录", notes = "更新购物车记录", httpMethod = "POST")
    @PostMapping("/updateOrderCartElement")
    public Response<List<OrderCycleHeaderOut>> updateOrderCartElement(@RequestBody List<CreateOrderSkuIn> shoppingCartInList) {
        String userName = UserUtil.getUserName();
        AppUserOut appUserOut = appUserService.getAppUserByUserCode(userName);
        if (Objects.isNull(appUserOut)) {
            return Response.error(SystemConstant.HTTP_UNAUTHORIZED, "不存在的用户");
        }
        return disShoppingCartService.updateShoppingCart(shoppingCartInList, appUserOut, false);
    }

    @ApiOperation(value = "是否可以加购", notes = "是否可以加购", httpMethod = "GET")
    @GetMapping("/isCanBuyFlashSaleGoods")
    public Response<FlashSaleCheckOut> isCanBuyFlashSaleGoods(String skuCode) {
        String bizOrgCode = UserUtil.getBizOrgCode();
        return Response.data(disShoppingCartService.isCanBuyFlashSaleGoods(skuCode, bizOrgCode));
    }
...
//        orderGoodsIn.setBizOrgCode(appUserOut.getBizOrgCode());
        ...
        //rpc 查询订单类型配置
        OrderTypeConfig optOrderTypeConfig = orderConfigHandle.getOneByCombinationTypeValueList(orderGoodsIn.getStoreCode(), bizOrgCode, combinationTypeCodeList);
        if (Objects.nonNull(optOrderTypeConfig)) {
            return Response.data(JSONObject.toJSONString(optOrderTypeConfig));
        }
