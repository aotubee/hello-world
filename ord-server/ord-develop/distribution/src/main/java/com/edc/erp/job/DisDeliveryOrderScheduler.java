...
@Component
@Slf4j
public class DisDeliveryOrderScheduler {
    ...
    public void autoTakeDelivery() {
        String closeAutoTakeDeliverySwitch = redisService.get(DisSystemConstant.DIS_CLOSE_AUTO_TAKE_DELIVERY_SWITCH_KEY);
        if (CLOSE_AUTO_TAKE_DELIVERY_KEY.equals(closeAutoTakeDeliverySwitch)) {
            log.info("自动收货开关未打开......");
            return;
        }
        log.info("开始执行自动收货任务");
        if (taskLockUtils.lock(DisSystemConstant.DIS_AUTO_TAKE_DELIVERY_SWITCH, 60 * 15 * 1000)) {
            try {
                ...
                takeDisDeliveryHandle.autoTakeDisDelivery();
            } catch (Exception e) {
                log.error("执行自动收货任务异常：", e);
            } finally {
                taskLockUtils.unlock(DisSystemConstant.DIS_AUTO_TAKE_DELIVERY_SWITCH);
            }
        } else {
            log.info("定时器锁还未释放：{}", DisSystemConstant.DIS_AUTO_TAKE_DELIVERY_SWITCH);
        }
        log.info("自动收货任务执行结束");
    }
