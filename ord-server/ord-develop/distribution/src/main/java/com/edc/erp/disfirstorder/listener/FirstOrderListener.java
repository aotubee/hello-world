package com.edc.erp.disfirstorder.listener;
 ...
 */
@Slf4j
public class FirstOrderListener extends ImportListener<ImportFirstOrderDetail> {
    ...
    private final List<OrdDisOrderFirstDetailOut> outDetails = new ArrayList<>();
    ...
    public FirstOrderListener(OrderGoodsServer orderGoodsServer
    ...
    public Response<List<OrdDisOrderFirstDetailOut>> getResponse() {
        /**按商品code对导入的数据去重*/
        List<OrdDisOrderFirstDetailOut> detailOuts = outDetails.stream().collect(
                Collectors.collectingAndThen(
                        Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(OrdDisOrderFirstDetailOut::getGoodsCode))), ArrayList::new)
        );
        String message = "";
    ...
    public List<OrdDisOrderFirstDetailOut> getOutDetails() {
        return outDetails;
    }
    ...
    private void initDetail(ImportFirstOrderDetail data, OrderGoodsOut goodsOut) {
        if (Objects.isNull(goodsOut.getDistributionPrice())) {
            throw new BusinessException(data.getGoodsCode() + "：此商品无配销价;");
        }
        //明细结果集
        OrdDisOrderFirstDetailOut detail = new OrdDisOrderFirstDetailOut();
    ...
    public void doAfterAllAnalysed(AnalysisContext context) {
        if (outDetails.size() >= BATCH_COUNT) {
            outDetails.clear();
            this.onException(new BusinessException("最大支持导入" + BATCH_COUNT + "条"), context);
        }
        log.info("FirstOrderListener解析完成");
    }

    @Override
    public void onException(Exception exception, AnalysisContext context) {
        log.error("FirstOrderListener解析失败，发生异常:{}", exception.getMessage(), exception);
