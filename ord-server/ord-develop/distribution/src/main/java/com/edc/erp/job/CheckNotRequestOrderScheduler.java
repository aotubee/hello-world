...
@Component
@Slf4j
public class CheckNotRequestOrderScheduler {
...
//    @Scheduled(cron = "0 0,10,20 * * * ?")
    public void sendMessageForNoTransferOrder() {
        ...
        log.info("开始执行查询特许加盟06:30未正常转单任务");
        String key = SystemConstant.TO_REQUEST_ORDER_ERROR_SWITCH + StoreOrderWarningTypeEnum.FRANCHISE.getCode();
            ...
            try {
                for (String bizOrgCode : SystemConstant.BIZORGCODES) {
                    ...
                    if (CollectionUtils.isEmpty(orderNoList)) {
                        log.info("{}特许加盟未正常转单定时器未查到数据", OrgCodeConvertEnum.getNameByBizOrgCode(bizOrgCode));
                        continue;
                    }
                    String msg = MessageFormat.format(OrderWarningTypeEnum.NOT_TO_REQUEST_ORDER.getErrorMessage(), StoreOrderWarningTypeEnum.FRANCHISE.getName(), truncationDateTime, String.join(SystemConstant.COMMA, orderNoList));
                ...
                log.error("执行查询特许加盟06:30未正常转单任务异常：", e);
            } finally {
                taskLockUtils.unlock(key);
            }
        } else {
            log.info("查询特许加盟06:30未正常转单定时器定时器锁还未释放：{}", key);
        }
        log.info("查询特许加盟06:30未正常转单任务执行结束");
    }
}
