 ...
 */
@Slf4j
public class DisDeliveryAsyncImportListener extends ImportListener<ImportDisDeliveryOrder> {
    ...
    public DisDeliveryAsyncImportListener() {
    ...
    public void invoke(ImportDisDeliveryOrder data, AnalysisContext context) {
        ...
        if (Objects.isNull(data.getDeliveryQuantity())) {
            ...
            if (NumberUtil.INTEGER_ZERO >= data.getDeliveryQuantity() || !cn.hutool.core.util.NumberUtil.isInteger(data.getDeliveryQuantity().toString())) {
                errorJoiner.add("列配销数量必须为大于0的正整数");
            }
        }
