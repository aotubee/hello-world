...
@Component
@Slf4j
public class CheckNotDeliveryOrderScheduler {
...
//    @Scheduled(cron = "0 10,20,30 * * * ?")
    public void sendMessageForNoTransferOrder() {
        ...
        log.info("开始执行查询特许加盟06:30未正常拆单任务");
        String key = SystemConstant.TO_DELIVERY_ORDER_ERROR_SWITCH + StoreOrderWarningTypeEnum.FRANCHISE.getCode();
            ...
            try {
                for (String bizOrgCode : SystemConstant.BIZORGCODES) {
                    ...
                    if (CollectionUtils.isEmpty(orderNoList)) {
                        log.info("{}特许加盟未正常拆单定时器未查到数据", OrgCodeConvertEnum.getNameByBizOrgCode(bizOrgCode));
                        continue;
                    }
                    String msg = MessageFormat.format(OrderWarningTypeEnum.NOT_TO_DELIVERY_ORDER.getErrorMessage(), StoreOrderWarningTypeEnum.FRANCHISE.getName(), WarningBusinessTypeEnum.DIS_REQUEST_ORDER.getName(),
                            WarningBusinessTypeEnum.DIS_REQUEST_ORDER.getName(), truncationDateTime, String.join(SystemConstant.COMMA, orderNoList));
                    warningService.pushWarningMessage(WarningBusinessTypeEnum.DIS_REQUEST_ORDER.getBusinessType(), OrderWarningTypeEnum.NOT_TO_DELIVERY_ORDER.getType(),
                            msg, null, null, bizOrgCode);
                }
            } catch (Exception e) {
                log.error("执行查询特许加盟06:30未正常拆单任务异常：", e);
            } finally {
                taskLockUtils.unlock(key);
            }
        } else {
            log.info("查询特许加盟06:30未正常拆单定时器定时器锁还未释放：{}", key);
        }
        log.info("查询特许加盟06:30未正常拆单任务执行结束");
    }
}
